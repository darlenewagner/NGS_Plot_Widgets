...
logger = logging.getLogger("sevenPooledPlotFastq.py")
...
parser = argparse.ArgumentParser(description='Computes sequence lengths and average PHRED for shuffled paired reads in fastq (Expects single fastq input)', usage="python sevenPooledPlotFastq.py filepath/filename1.fastq(.gz) filepath/filename2.fastq(.gz) filepath/filename3.fastq(.gz) filepath/filename4.fastq(.gz) filepath/filename5.fastq(.gz) filepath/filename6.fastq(.gz) filepath/filename7.fastq(.gz) --outputType [F/J/N]")
...
parser.add_argument('--title', '-t', default='Norovirus', help="Title for plot and output file names, if applicable")
...
avgFwdAvgPHRED = np.mean(lisMedFwdAvgPHRED)
print(avgFwdAvgPHRED)
minFwdAvgPHRED = min(lisMedFwdAvgPHRED)
...
avgRevAvgPHRED = np.mean(lisMedRevPHRED)
print(avgRevAvgPHRED)
minRevAvgPHRED = min(lisMedRevPHRED)
...
axes[0].hist(lisMedFwdAvgPHRED, bins = 25, color='blue')
axes[0].set_title("R1 Quality " + newTitle)

## Plot PHRED Quality for R2 reads as 1D histogram
axes[1].hist(lisMedRevPHRED, bins = 25, color='red')
