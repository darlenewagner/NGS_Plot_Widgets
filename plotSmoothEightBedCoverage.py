...
myCoverage15 = open(args.filename15.name, "r")

for line in myCoverage15:
        lineData = re.split(r'\s+', line)
        referenceName = lineData[0]
        if(iter % window == 0):
                coordinates15.append(iter)
                # add plotting point to smoothing window
                smooth15.append(int(lineData[2]))
                # take average of smoothing window, 5, 10, or 20 points
                coverage15.append(statistics.mean(smooth15))
                smooth15 = []
        else:
                smooth15.append(int(lineData[2]))
        iter = iter + 1
...
myCoverage16 = open(args.filename16.name, "r")

for line in myCoverage16:
        lineData = re.split(r'\s+', line)
        referenceName = lineData[0]
        if(iter % window == 0):
                coordinates16.append(iter)
                # add plotting point to smoothing window
                smooth16.append(int(lineData[2]))
                # take average of smoothing window, 5, 10, or 20 points
                coverage16.append(statistics.mean(smooth16))
                smooth16 = []
        else:
                smooth16.append(int(lineData[2]))
        iter = iter + 1
...
axes.set_title("Raw Polio " + shortTitle1t[0] + ", " + shortTitle2t[0] + ", " + shortTitle3t[0] + ", " + shortTitle4t[0] + ", " + shortTitle5t[0] + ", " + shortTitle6t[0] + ", " + shortTitle7t[0] + ", and " + shortTitle8t[0], size=14)
axes.set_xlabel('Reference Genome, ' + referenceName + ', Coordinates', size=14)
axes.set_ylabel('Coverage (X) at Position', size=14)
#axes.margins(0.2)

fig.savefig('/scicomp/home-pure/ydn3/test_Python3.9.1/test_Biopython/rawSmooth_w' + str(window) + '_' + shortTitle1t[0] + '_' + shortTitle2t[0] + '_' + shortTitle3t[0] + '_' + shortTitle4t[0] + '_' + shortTitle5t[0] + '_' + shortTitle6t[0] + '_' + shortTitle7t[0] + '_' + shortTitle8t[0] + '_to_' + referenceName + '.png')
