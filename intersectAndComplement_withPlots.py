...
uniqueFile1 = Compl(snpsFile1, compPositions)
uniqueFile2 = Compl(snpsFile2, compPositions)
...
if(args.outputType == 'C'): ### output single column of positions, intersection or union
    ...
    for item in columns1[stats[0]]:
        intStats1.append(int(item))
        #print(item)

    #coverage = []
    intStats2 = []
    for item in columns2[stats[0]]:
        intStats2.append(int(item))
        #print(item)

    fig, axes = plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False, figsize=(8.5,11))
    axes[0].stem(allPositions, intStats1)
    axes[0].set_title(args.listFile1.name)
    axes[0].set_ylabel('SNP Quality')
    axes[1].stem(allPositions, intStats2)
    axes[1].set_title(args.listFile2.name)
    axes[1].set_xlabel('Reference Position')
    axes[1].set_ylabel('SNP Quality')
    fig.savefig('/scicomp/home-pure/ydn3/test_Python3.9.1/test_pandas/try_lollipop6.png')
